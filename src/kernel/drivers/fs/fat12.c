...
static void fat12_parse_fname(const char *in, char *parsed)
{
        char        c;
        const char *p;
        int         i;
...
static ion_errcode fat12_parse_header(struct disk *drive, fat12_header_t *out)
{
        char        buff[513];
        int         i;
        ion_errcode err;
...
static void fat12_parse_entry(fat12_entry *entry, const char *buff)
{
        int i;
        ...
        for (i = 0; i < sectors_in_dir; i++) {
                ...
                for (j = 0; j < entries_per_sector; j++) {
                        if (0 == *(char *)buff + j * FAT12_ENTRY_LEN)
                                goto notfound;
                        fat12_parse_entry(&dir,
                                          (char *)buff + j * FAT12_ENTRY_LEN);
...
static int fat12_find(ion_fs *fat12, ion_file *file, const char *path)
{
        const char      *next;
        char             currname[13], pfname[12];
        char            *buff;
        int              i, j, is_root_dir;
        int              entries_per_sector;
        ...
        buff = ion_malloc_e(info->sector_size, &err);
        if (err.number)
                return err.number;
...
/**/
static void fat12_tofattime(struct ion_time *ion_time, struct fat_time *dos)
{
        dos->time = ion_time->hour << 11;
        dos->time |= ion_time->minute << 5;
        dos->time |= (ion_time->second / 2 & 0x1f);
        dos->date = (ion_time->year + 2000 - 1980) << 9;
        dos->date |= (ion_time->month & 0xf) << 5;
        dos->date |= (ion_time->day & 0x1f);
        dos->ms = ion_time->ms;
}
...
static int fat12_next_cluster(ion_fs *fat12, int prev_cluster)
{
        ...
        info = (struct _fat12_mountinfo *)fat12->meta;
        FAT  = ion_malloc_e(info->sector_size * 2, &err);
        if (err.number)
                goto cleanup;

        fat_offset = prev_cluster + (prev_cluster / 2);
...
static int fat12_update_entry(ion_file *file, int option)
{
        ...
        fat12 = Ion_filesystems[file->device];
...
static int fat12_create_entry(ion_fs *fat12, fat12_entry *entry,
                              const char *path)
{
        int         i;
        const char *fname;
        char        rest_of_path[100];
        char        sector[512], entrybuff[FAT12_ENTRY_LEN];
...
ion_errcode fat12_mount(ion_fs *fat12, struct disk *device)
{
        ...
        info = (struct _fat12_mountinfo *)ion_malloc_e(sizeof(fat12_mountinfo),
                                                       &err);
...
ion_errcode fat12_read(ion_file *file, void *buff, usize count)
{
        ...
        data = ion_malloc_e(cluster_size, &err);
        if (err.number) {
                file->flags = ION_FS_INVALID;
                return err.number;
        }
        ...
        for (i = 0; i < clusters_to_read; i++) {
                ...
                if (count < cluster_size && !fat12_dont_read) {
                        memcpy((u8 *)buff + (i + 1) * cluster_size, data,
                               count);
                        break;
                } else {
                        if (!fat12_dont_read) {
                                memcpy((u8 *)buff + (i + 1) * cluster_size,
                                       data, cluster_size);
                        }
                        count -= cluster_size;
                }
...
ion_errcode fat12_open(ion_file *outfile, struct disk *device, const char *path)
{
        struct ion_error err;
        ion_fs          *fat12;
        ion_file         file;

        fat12 = Ion_filesystems[device->dev];
        if (fat12 == NULL) {
...
ion_errcode fat12_write(ion_file *file, const void *buff, usize count)
{
        ...
        sector_index = info->data_area_offset +
                       (file->cluster - 2) * info->sectors_per_cluster;
        cluster_size = info->sectors_per_cluster * info->sector_size;
        cluster = ion_malloc_e(info->sectors_per_cluster * info->sector_size,
                               &err);
        if (err.number)
                goto cleanup;
        ...
        if (pos_in_cluster + count < cluster_size) {
                ...
                if (fat12_write_zero)
                        ...
                        memcpy(cluster, (u8 *)buff + curr, len);
                curr += len;
                err.number = disk_store_sectors(&fat12->device, sector_index,
                                                info->sectors_per_cluster,
                                                cluster);
